...
| `admin.packetcraft.dev` Access application and identity policy | Cloudflare Access |
...
The administrator uses the first-level `admin.packetcraft.dev` hostname so it
is covered by the zone's Universal SSL certificate. Do not restore the nested
`admin.site.packetcraft.dev` name unless the zone gains explicit multi-level
...
- the intended admin ingress is `admin.packetcraft.dev/api/admin/*`; Cloudflare must protect the entire admin hostname
